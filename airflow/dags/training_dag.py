...
volume_mount = k8s.V1VolumeMount(name="training-storage", mount_path="/var/tmp/", sub_path=None)
IMAGE = "rostyslavskliar/garbage-classifier-trainer:latest"

with DAG(start_date=datetime(2023, 1, 1), catchup=False, schedule_interval=None, dag_id="training_dag") as dag:
    ...
    load_data = KubernetesPodOperator(
        ...
        cmds=["python", "garbage_classifier/cli.py", "load-train-data",
                 s3_access_key, s3_secret_key, s3_bucket, s3_prefix, "/var/tmp/data"],
        task_id="load_data",
        in_cluster=False,
        namespace="default",
        volumes=[volume],
    ...
    train_model = KubernetesPodOperator(
        ...
        cmds=["python", "garbage_classifier/cli.py",
                 "train", "garbage_classifier/data/config.json", "/var/tmp/data/train.tar.gz", "/var/tmp/data/test.tar.gz"],
        task_id="train_model",
        in_cluster=False,
        namespace="default",
        volumes=[volume],
    ...
    upload_model = KubernetesPodOperator(
        ...
        image=IMAGE,
        cmds=["python", "garbage_classifier/cli.py",
                 "upload-to-registry", "garbage-classifier-model", "/var/tmp/data/model/"],
